<template>
    <div class="page page-no-radius">
        <header-bar title="申诉" :border="false" :back="-1" :isIos="iosShow"></header-bar>
        <van-cell-group>
            ...
            <van-action-sheet v-model="isShow" @select="onSelect" title="申诉类型" cancel-text="关闭" :border="true">
                <van-cell-group>
                    <van-cell v-for="item in list.type_list" :key="item.value" :border="true" @click="onSelect(item)">
                        <template #title>
...
export default {
    ...
    methods: {
        fetchData() {
            ...
            this.$apps.http
                .get("/SoldService.Order/goods_detail", { order_goods_id: this.$route.params.prodId })
                .then((res) => {
                    if (res.code == 1) {
                        this.list = res.data;
                    }
        ...
        submitAction() {
            ...
            let t = this;
            this.$apps.http
                .post("/SoldService.Order/appeal", formdata)
                .then((res) => {
                    this.loading = false;
                    if (res.code == 1) {
                        this.$toast(res.msg);
                        setTimeout(() => {
                            t.$router.replace("/member/service/1");
                            t.$store.dispatch(
                                "SET_LOCALSTORAGE",
                                JSON.stringify({
                                    name: "SERVICEBAR",
