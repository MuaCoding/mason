...
export default {
    ...
    created() {
        this.GET_SERVICEBAR && (this.flag = !this.flag);
        if (this.flag) {
            this.active = Number(this.GET_SERVICEBAR);
        } else {
    ...
    methods: {
        ...
        fetchData(key, page) {
            ...
            try {
                ...
                this.$apps.http
                    .get("/SoldService.Order/index", formdata)
                    .then((result) => {
                        if (result.code === 1) {
                            switch (key) {
                                case 0:
        ...
        onChange(index, page) {
            this.SET_LOCALSTORAGE(JSON.stringify({ name: "SERVICEBAR", value: this.active }));
            page = parseInt(page) ? parseInt(page) : 1;
            this.fetchData(this.active, page);
        },
        goto(item, route) {
